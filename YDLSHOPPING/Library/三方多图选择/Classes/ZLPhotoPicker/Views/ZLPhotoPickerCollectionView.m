...
//        NSString *format = [NSString stringWithFormat:@"现在还能上传%zd张图片",maxCount];
        NSString *format = [NSString stringWithFormat:@"You have selected all the pictures."];
        if (maxCount <= 0) {
            format = [NSString stringWithFormat:@"You have selected all the pictures."];
        }
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Tips" message:format delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
        [alertView show];
        return NO;
    }
    return YES;
