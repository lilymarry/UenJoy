...
- (void)longGesture:(UILongPressGestureRecognizer *)gesture{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (!self.sheet) {
            self.sheet = [[UIActionSheet alloc] initWithTitle:@"Tips" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:@"Save to album" otherButtonTitles:nil, nil];
        }
    ...
    if (buttonIndex == 0){
        if([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeSavedPhotosAlbum]) {
            ...
            if (_photoImageView.image) {
                [self showMessageWithText:@"No user permission, saving failed"];
            }
        }
    }
}
