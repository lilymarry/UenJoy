...
- (void) setup{
    ...
    [cancalBtn setTitle:@"Cancell" forState:UIControlStateNormal];
...
- (void)showPickerVc:(UIViewController *)vc{
    ...
    if(status == AVAuthorizationStatusDenied){
        ...
        NSString *title = nil;
        NSString *msg = @"Photo album permission has not been opened ~ please open it in system settings";
        NSString *cancelTitle = @"No";
        NSString *otherButtonTitles = @"To set";
    
        if (kSystemMainVersion < 8.0) {
            title = @"Album permission is not turned on";
            msg = @"Please turn on camera service in system \n settings  (Settings > Privacy > album > on)";
            cancelTitle = @"Got it";
...
- (void)stillImage:(id)sender
{
    ...
    if (self.maxCount > 0 && self.images.count >= self.maxCount) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Tips" message:@"The number of pictures cannot be exceeded" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
