...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.bannerImageType == bannerImageTypeNetworkURL) {
          NSString* encodedString =[self.networkImageURLs[indexPath.item] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];//URL 含有中文 encode 编码
        
        [cell.bannerImageView sd_setImageWithURL:[NSURL URLWithString:encodedString] placeholderImage:self.placeholderImageName.length?[UIImage imageNamed:self.placeholderImageName]:nil];


    }
    if (self.bannerImageType == bannerImageTypeNetworkModel) {
     
             NSString* encodedString =[[self fetchModelUrl:indexPath.item] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];//URL 含有中文 encode 编码
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    NSString *num1=[NSString stringWithFormat:@"%ld",(long)self.pageControl.currentPage+1];
    NSString *num2=[NSString stringWithFormat:@"%ld",(long)_totalPages];
    
    NSMutableAttributedString * AttributedStr = [[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"%@ /%@",num1,num2]];
    
    [AttributedStr addAttribute:NSForegroundColorAttributeName value:Color(@"#333333") range:NSMakeRange(0,num1.length)];
    
    if (_isNumLab) {
      self.pageNumLab.attributedText=AttributedStr;
    }
    if (!_isPageControl) {
...
{
    if (!_pageNumLab) {
        _pageNumLab = [[UILabel alloc] initWithFrame:CGRectMake(12, self.bounds.size.height - self.pageControlBottom-20, self.bounds.size.width-24, 20.f)];
        _pageNumLab.font=font(12);
        _pageNumLab.textColor=Color(@"#999999");
        _pageNumLab.backgroundColor=[UIColor clearColor];
        [self addSubview:_pageNumLab];
    }
    return _pageNumLab;
}
- (NSCache *)cache {
