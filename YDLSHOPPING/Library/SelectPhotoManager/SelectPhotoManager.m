...
- (void)startSelectPhotoWithImageName:(NSString *)imageName{
    ...
    [alertController addAction: [UIAlertAction actionWithTitle: @"Choose from album" style: UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
...
- (void)startSelectPhotoWithType:(SelectPhotoType)type andImageName:(NSString *)imageName {
    ...
    if (type == PhotoCamera) {
        ...
        if (!isCamera) {
            NSLog(@"没有摄像头");
            if (_errorHandle) {
                _errorHandle(@"No Camera");
            }
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"Tips" message:@"Your device does not support taking photos" preferredStyle:UIAlertControllerStyleAlert];
